    ...
    try:
        ...
        try:
            ...
            with col1:
                ...
                Odd_H_Max = st.number_input('Odd_H_Max', value=100.00, step=0.1)
            with col2:
            ...
            # Criando o Filtro do Método de Lay CS - Favorito Away
            ...
            df_Away = Jogos[flt1]
            df_Away = df_Away[['League','Time','Home','Away','Odd_H','Odd_D','Odd_A','Odd_Over25','Odd_BTTS_Yes','IDMercado','IDEvento']]
            df_Away = drop_reset_index(df_Away)
        except:
            pass
        ...
        if len(df0) != 0:
            ...
            try:
                ...
                j1, j2 = st.columns(2)
                selecao1 = j1.selectbox('Escolha o Visitante', lista_A) 
                selecao2 = j2.selectbox('Escolha o Placar', lista)
                
                Favorito = selecao1
                Placar = selecao2
                
                df_league = df_Away[df_Away.Away == Favorito].copy()
                row = df_Away.loc[df_Away['Away'] == Favorito].iloc[0]
            ...
            try:
                ...
                # Home
                sing_H = df_sin2[df_sin2.Home == Zeb_Home].copy()
                sing_H = sing_H[sing_H[Placar] == 1]
                n_sing_H = sing_H[sing_H.columns[0]].count()

                # Away
                sing_A = df_sin2[df_sin2.Away == Fav_Away].copy()
                sing_A = sing_A[sing_A[Placar] == 1]
                n_sing_A = sing_A[sing_A.columns[0]].count()

                # Home x Away
                sing_HxA = df_sin2[(df_sin2.Home == Zeb_Home) & (df_sin2.Away == Fav_Away)].copy()
            ...
            try:
                ...
                # Odd Média do Time
                ################################################################################    
                df_Odd_Med = df[df.Away == Favorito]
                ...
                ################################################################################
                odd_atual = Jogos[Jogos.Away == Fav_Away].copy()
                placar = 'Odd_'+ Placar
                odd_betfair = odd_atual[placar].tail(1).item()
                st.text("Odd Atual do Lay " + Placar + " do " + Fav_Away + " na Betfair é: " + str(odd_betfair))
            ...
            try:
                ...
                last = df_CG[df_CG['Home'] == Zeb_Home].index[-1]
                ...
                last = df_CG[df_CG['Away'] == Fav_Away].index[-1]
                ...
                st.text(f'{Zeb_Home} - IF: {round(media_cg_h,2)}, CV: {round(cv_cg_h,2)}')
                st.text(f'{Fav_Away} - IF: {round(media_cg_a,2)}, CV: {round(cv_cg_a,2)}')

                st.text('')
                st.text('')

                st.text(f'Coeficiente de Eficiência (CE) - Ultimos {n_per} Jogos')
                st.text(f'{Zeb_Home} - CE: {round(media_cg2_h,2)}, CV: {round(cv_cg2_h,2)}')
                st.text(f'{Fav_Away} - CE: {round(media_cg2_a,2)}, CV: {round(cv_cg2_a,2)}')
            ...
            try:
                ...
                ################################################################################  
                # Home
                sing_H1 = df[df.Home == Zeb_Home].copy()
                ...
                sing_A1 = df[df.Away == Fav_Away].copy()
                ...
                # Home x Away
                sing_HxA1 = df[(df.Home == Zeb_Home) & (df.Away == Fav_Away)].copy()
            ...
            try:
                ...
                #     # Home
                #     df_home1000 = df[(df['Home'] == Zeb_Home)]
                #     df_filtrado = df_home1000[(df_home1000[selecao_full] == 1)]
                #     df_filtrado = drop_reset_index(df_filtrado)
                #     n_H = len(df_filtrado)
                #     df1000_H = df[df.Home == Zeb_Home]
                ...
                #     # Away
                #     df_away1000 = df[(df['Away'] == Fav_Away)]
                #     df_filtrado = df_away1000[(df_away1000[selecao_full] == 1)]
                #     df_filtrado = drop_reset_index(df_filtrado)
                #     n_A = len(df_filtrado)
                #     df1000_A = df[df.Away == Fav_Away]
            ...
            try:
                ...
                df_10_jogos_H = df[df.Home == Zeb_Home].tail(10)
            ...
            try:
                ...
                df_10_jogos_A = df[df.Away == Fav_Away].tail(10)
            ...
            try:
                ...
                df_Confronto = df[((df.Home == Zeb_Home) & (df.Away == Fav_Away))]
